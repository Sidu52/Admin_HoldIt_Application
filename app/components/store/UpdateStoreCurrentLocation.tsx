...
interface UpdateStoreCurrentLocationProps {
    store: Store;
...
function formatTimestamp(date: Date | null | string): string { // FIX 3: Handle string dates
    if (!date) return "—";
...
export default function UpdateStoreCurrentLocation({
    store,
    ...
    className = "",
}: UpdateStoreCurrentLocationProps) {
    const existing = store.location;
    ...
    function validate(lat: string, lng: string): boolean {
        ...
        if (!lat.trim() || isNaN(latN)) {
            errors.lat = lat.trim() ? "Must be a valid number" : "Required";
        } else if (latN < -90 || latN > 90) {
            errors.lat = "Must be between -90 and 90";
        }

        if (!lng.trim() || isNaN(lngN)) {
            errors.lng = lng.trim() ? "Must be a valid number" : "Required";
    ...
    const handleDetectLocation = useCallback(async () => {
        ...
        try {
            ...
            if (perm === "prompt") {
                setErrorMsg("Please allow location access when prompted.");
            }
            ...
            const errorMessage = err instanceof Error ? err.message : "Unable to retrieve location. Please check your GPS settings.";
    ...
    const handleSubmit = useCallback(async () => {
        ...
        if (isNaN(lat) || isNaN(lng)) {
            setErrorMsg("Invalid latitude or longitude values");
            setStatus("error");
            return;
        }
        ...
        try {
            ...
            const now = new Date();
            setLocation(prev => ({ ...prev, lat, lng, address, updatedAt: now }));
            setStatus("success");
            ...
            console.error("Update location error:", err); // FIX 13: Added error logging
    ...
    }, [latInput, lngInput, addressInput, store._id, updateLocationMutation, onSuccess, onError]);
    ...
    return (
        ...
        >
            ...
            <button
                ...
                disabled={isBusy || (!latInput.trim() && !lngInput.trim())} // FIX 15: Disable if no coordinates
